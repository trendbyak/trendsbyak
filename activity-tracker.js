...
document.addEventListener('click',e=>{const b=e.target.closest('button,a');if(!b)return;if(/add.*cart/i.test((b.textContent||'')+' '+(b.className||''))){event('add_to_cart_click');setTimeout(syncCart,50)}if(/checkout|proceed to checkout/i.test(b.textContent||'')){event('checkout_started');setTimeout(syncCart,50)}});
