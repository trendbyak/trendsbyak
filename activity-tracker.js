...
async function identify(data){if(!data)return null;const clean={full_name:data.name||data.full_name||null,email:data.email||null,phone:data.phone||data.mobile||null,whatsapp_phone:data.whatsapp_phone||data.phone||data.mobile||null};if(!clean.email&&!clean.phone)return null;try{const r=await db.rpc('record_customer',{p:clean});if(r.error)throw r.error;customerId=r.data||null;customerProfile={...clean};write('tak_customer',customerProfile);if(customerId){try{await db.from('analytics_events').update({customer_id:customerId}).eq('session_id',session.id).is('customer_id',null)}catch{}}return customerId}catch(e){console.debug('customer identify',e);return null}}
...
document.addEventListener('click',e=>{const b=e.target.closest('button,a');if(!b)return;if(/add.*cart/i.test((b.textContent||'')+' '+(b.className||''))){event('add_to_cart_click');setTimeout(syncCart,100)}if(/checkout|proceed to checkout/i.test(b.textContent||'')){event('checkout_started');setTimeout(syncCart,100)}});
document.addEventListener('input',e=>{const id=e.target?.id||'';if(['name','email','phone'].includes(id)){const d={name:document.getElementById('name')?.value?.trim(),email:document.getElementById('email')?.value?.trim(),phone:document.getElementById('phone')?.value?.trim()};if(d.name||d.email||d.phone){customerProfile={full_name:d.name||null,email:d.email||null,phone:d.phone||null};write('tak_customer',customerProfile);clearTimeout(window.__takIdentifyTimer);window.__takIdentifyTimer=setTimeout(()=>identify(d).then(()=>syncCart()),700)}}});
